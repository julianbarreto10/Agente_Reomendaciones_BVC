...
def stock_history(stock,start_date,end_date,tp=0):
    ...
    with open("Scrapings/scraping_stocks/info_accion.txt", "w") as file:
        file.write(descripcion)

    # Exportar a un archivo CSV
    if tp==0:
        df.to_csv("Scrapings/scraping_stocks/data_stock.csv")
    else:
        df.to_csv("Resultados_Experimento"+stock+"/data_stock.csv")